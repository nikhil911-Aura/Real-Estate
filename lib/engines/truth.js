...
async function runTruthEngine(filePath, onProgress) {
  ...
  // Step 1: Parse CSV and extract only needed fields
  const rows = await parseCsvRobust(filePath);
  rowCount = rows.length;

  for (let i = 0; i < rows.length; i++) {
    const slim = extractFields(rows[i]);
    rows[i] = null; // free the full row immediately

    const normAddr = normalizeAddress(slim.address);
    if (!normAddr || !slim.zip) continue;
