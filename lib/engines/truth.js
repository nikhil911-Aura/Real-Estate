...
async function runTruthEngine(filePath, onProgress) {
  ...
  if (onProgress) onProgress(2, 'Reading CSV file...');
  ...
  rowCount = await parseCsvStream(filePath, (row) => {
    ...
    // CSV parsing is 0-30% of total progress
    const progress = Math.round(pct * 0.3);
    if (onProgress) onProgress(progress, `Parsing CSV... ${rows.toLocaleString()} rows`);
  });

  if (onProgress) onProgress(30, `Parsed ${rowCount.toLocaleString()} rows, finding flip pairs...`);
